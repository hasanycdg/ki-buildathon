...
const hotelContext = fs.readFileSync(path.join(rootDir, "hotel_alpenblick_kontext.md"), "utf8");
const quickHelpKnowledge = JSON.parse(
  fs.readFileSync(path.join(rootDir, "src/data/quickHelpKnowledge.json"), "utf8")
);
...
export function payloadFromRequest(request) {
...
export function parseLessonDraftJson(text) {
  const start = text.indexOf("{");
  const end = text.lastIndexOf("}");
  if (start === -1 || end <= start) {
...
export function quickHelpInstructions() {
  return [
    ...
    "Bei Bild-Uploads sollen die Schritte meistens enthalten: Foto machen/behalten, Problem nicht selbst reparieren, Fund/Schaden/Unklarheit der Hausdame, Schichtleitung oder Rezeption intern 100 melden enthalten.",
...
export function lessonDraftInstructions() {
  return [
...
export function buildQuickHelpUserContent({ question, attachment, history }) {
...
export async function openAiResponse({ instructions, input, temperature, maxOutputTokens }) {
